...
Prompts for orchestrating multi-agent parallel work in Codex. Each prompt creates a team of specialists that work simultaneously and communicate with each other.

**Prerequisites:** Agent teams must be enabled in your `~/.codex/settings.json` (already done if you followed the install guide). Restart Codex after enabling.

**Cost awareness:** Agent teams use significantly more tokens than single sessions. Each teammate is a separate Codex instance. Use them when parallel work genuinely saves time — not for simple sequential tasks.
...
node "$HOME/.codex/plugins/cache/openai-codex/codex/1.0.4/scripts/codex-companion.mjs" \
  ...
  "Adversarially evaluate against the contract in [PLAN PATH]. Use only the contract, diff, and AGENTS.md."
If adversarial evaluation is disabled in config, skip this step and record that in the synthesis.
...
not "pretty good." Score lower when in doubt.
...
Use this for Tier 3 contracts — architectural changes, new pipeline phases, or complex features where maximum evaluation rigor is needed. Spawns separate primary-evaluator passes that challenge the implementation from different angles, **plus** a parallel configured adversarial review for cross-model coverage.
...
1. Contract evaluator — read the contract JSON from the plan file. For EACH
   ...
   1-10. You are NOT the implementer. Do NOT be generous. A 7 means
   functional but insufficient, not "pretty good." Score lower when in doubt.
...
- Each evaluator reads only: the contract, git diff, and AGENTS.md
...
1. Project architect — set up the directory structure, base configuration
   ...
   (logging, error handling, env config). Follow conventions from AGENTS.md.
