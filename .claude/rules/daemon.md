...
- **`ManifestStatus::Queued` is only ever observed on disk**; the orchestrator transitions `Queued → InProgress` as its first action after acquiring the global provider permit. A dispatch-time crash before transition leaves a `Queued` manifest; startup reconciliation treats these as DELETE (system-level state repair, not an audit-worthy interrupt).
