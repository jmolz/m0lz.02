...
**Core Functionality**
- [ ] `pice init` — scaffold `.codex/` directory with all PICE framework files (commands, templates, hooks, docs, rules)
...
Initializes a project with the complete PICE framework. Creates `.codex/` directory with all commands, templates, hooks, docs, and rules. Creates `.pice/` directory for local config and metrics. Detects existing `.codex/` and offers to merge rather than overwrite. Detects project type (language, framework) and customizes templates accordingly.
...
All evaluators see ONLY the contract, code diff, and AGENTS.md — never the planning or implementation context. This eliminates self-evaluation bias. The Rust core orchestrates multiple provider sessions in parallel and synthesizes results into a unified report.
...
Data sourced from `.pice/metrics.db` and filesystem scan of `.codex/plans/`.
...
| `evaluate/create` | Create an adversarial evaluation session (isolated context: contract + diff + AGENTS.md only) |
...
{"jsonrpc": "2.0", "method": "response/complete", "params": {"sessionId": "abc-123", "result": {"planPath": ".codex/plans/auth.md", "contractTier": 2}}}
```
...
- [ ] Template engine — embed `.codex/` framework files, scaffold on `pice init`
...
**Validation:** `pice init` creates a correct `.codex/` directory. `pice --help` shows all commands. Provider host can spawn and communicate with the stub provider.
...
3. **The existing `.codex/` command prompts translate cleanly to orchestrated sessions** — the current slash commands were written for interactive use. Orchestration may require prompt adjustments for programmatic consumption.
