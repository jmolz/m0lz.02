...
- **Daemon-spawning full validation must make the debug daemon discoverable and serialize Rust tests.** `cargo test --workspace --all-targets` includes pice-cli autostart/socket tests that invoke `pice-daemon`; run it with `PATH="$PWD/target/debug:$PATH"` locally or add `$GITHUB_WORKSPACE/target/debug` to `GITHUB_PATH` in CI, and set `RUST_TEST_THREADS=1`. Phase 8 self-heal anchor: the plain command failed in `adapter::transport::*`, `adapter::autostart::*`, and `commands::daemon::*`; the PATH-qualified serialized command passed.
