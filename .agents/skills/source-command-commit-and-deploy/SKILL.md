...
**Expected baseline:** 1262 Rust tests (1 ignored doc-test), 124 TypeScript tests, 0 lint errors, 0 warnings, clean release build. The 1 ignored test is the doc-test in `crates/pice-daemon/src/handlers/mod.rs` (line 5). When the baseline shifts, update both this file AND `AGENTS.md` in the same release.
...
If any AI layer files changed (AGENTS.md, .codex/rules/, .agents/skills/source-command-), add a `Context:` section to the commit body.
