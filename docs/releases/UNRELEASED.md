...
| release workflow tripwire | `scripts/acceptance/release-workflow-policy.test.mjs` asserts that tag releases require npm publish, tag names match npm package versions, manual dry-runs do not publish, and publishable package versions stay aligned. | Covered by `pnpm test` and the targeted review selector. |
