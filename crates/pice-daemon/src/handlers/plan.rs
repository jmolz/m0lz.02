...
pub async fn run(
    ...
    let memory = if config
        .memory
        .policy()
        .can_read(pice_core::memory::MemoryConsumer::Plan)
    {
        ...
        let recall = build_memory_recall(
            &paths,
            &config.memory,
            pice_core::memory::MemoryConsumer::Plan,
            None,
            None,
        ...
        record_read_metrics(
            ...
            &paths,
            MemoryReadMetrics {
                consumer: pice_core::memory::MemoryConsumer::Plan,
                feature_id: None,
                plan_path: None,
                run_id: None,
    ...
    let prompt = builders::build_plan_prompt(
        &req.description,
        project_root,
