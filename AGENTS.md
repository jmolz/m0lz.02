...
- `.codex/PRD.md` — the v0.1 MVP PRD (shipped, 217 tests, treat as historical baseline)
...
| TypeScript 5.x | Provider implementations — Codex SDK bridge, Codex evaluator |
...
│   │       ├── config/        # .pice/ and .codex/ management
...
│    ↕ JSON-RPC over stdio
└── Providers (TS) ── Codex, Codex, future community providers
```
...
**Provider contract:** Providers declare capabilities (`workflow`, `evaluation`, or both) during `initialize`. The core routes commands accordingly — workflow to Codex, evaluation to both.
...
Evaluators see ONLY: contract JSON, git diff, AGENTS.md. Never implementation context.
...
| Area | File | When |
|------|------|------|
| Rust core | `.codex/rules/rust-core.md` | Working on crates/ |
| TypeScript providers | `.codex/rules/providers.md` | Working on packages/ |
| JSON-RPC protocol | `.codex/rules/protocol.md` | Changing provider contract or daemon RPC |
| Metrics & telemetry | `.codex/rules/metrics.md` | Working on metrics engine, audit trail, cost tracking |
| Templates & scaffolding | `.codex/rules/templates.md` | Changing `pice init` output |
| Visual assets & diagrams | `.codex/rules/docs-visual-assets.md` | Working on docs/images/, docs/diagrams/, or README visuals |
| Release & publishing | `.codex/rules/release.md` | Changing release workflows, version manifests, npm packages, tags, or release command guidance |
| **Daemon architecture (v0.2+)** | `.codex/rules/daemon.md` | Working on `pice-daemon`, `pice-core`, CLI adapter, socket transport, manifest |
| **Stack Loops (v0.2+)** | `.codex/rules/stack-loops.md` | Working on layer detection, DAG orchestration, worktree isolation, seam checks, context isolation |
| **Workflow YAML + adaptive + gates (v0.2+)** | `.codex/rules/workflow-yaml.md` | Working on `.pice/workflow.yaml`, SPRT/ADTS/VEC, approval gates |

For deep architecture reference: `.codex/docs/`
...
- **Evaluation sessions are context-isolated** — evaluator prompts must NEVER include implementation conversation or planning rationale. Only: contract, diff, AGENTS.md. In v0.2+, the rule extends to **per-layer context isolation**: a layer evaluator only sees its own layer's contract + filtered diff + AGENTS.md, never other layers' contracts, findings, or plan rationale.
